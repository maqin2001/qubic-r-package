...
template<typename T> NumericMatrix from_vector(const std::vector<std::vector<T>>& result) {
  int nc = result.size();
...
template<typename T> std::vector<std::vector<T>> to_vector(const NumericMatrix& matrix) {
  auto nc = matrix.ncol();
  auto nr = matrix.nrow();
